...
func ProjectPoolProposalOpenActive(c *fiber.Ctx, db *gorm.DB) error {
	offset, err := strconv.Atoi(c.Query("offset"))
	limit, err := strconv.Atoi(c.Query("limit"))
	...
	poolsResult := db.Debug().Where("status = ?", "Active").Or("status = ?", "Open").Preload("TierList", "deleted_at IS NULL").Preload("PollList", "deleted_at IS NULL").Limit(limit).Order("created_at desc").Offset(offset).Find(&projectPools)
